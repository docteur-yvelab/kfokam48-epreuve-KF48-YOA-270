...
import type { RelectureEnAttente, ExerciceDetail, Etudiant } from '../types';
...
export default function RelecturePage() {
  ...
  const relectureId = id ? parseInt(id, 10) : null;
  ...
  // Charger l'exercice quand on a le relectureId
  useEffect(() => {
    if (relectureId && relecturesEnAttente) {
      const found = relecturesEnAttente.find((r: RelectureEnAttente) => r.id === relectureId);
      if (found) {
        // On a besoin de charger les détails de l'exercice
        // Pour l'instant on utilise les données de la liste
      }
    }
  }, [relectureId, relecturesEnAttente]);

  // Charger les détails de l'exercice via useExercices
  const { data: exercicesData } = useExercices(0, 0); // placeholder, on chargera via l'id

  useEffect(() => {
    if (relectureId && relecturesEnAttente) {
      const relecture = relecturesEnAttente.find((r: RelectureEnAttente) => r.id === relectureId);
      if (relecture) {
        // Fetch exercice detail
        fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8080'}/api/exercices/${relecture.exerciceId}`)
          .then(res => res.ok ? res.json() : null)
          .then(data => data && setExercice(data))
          .catch(console.error);
      }
    }
  }, [relectureId, relecturesEnAttente]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!relectureId || note === '' || !commentaire.trim()) return;
    clearError('soumettreRelecture');
    try {
      await soumettreRelecture.mutateAsync({ id: relectureId, data: { note: Number(note), commentaire: commentaire.trim() } });
      navigate('/etudiant');
    } catch (err) {
      // handled by hook
    }
  ...
  const relectureActuelle = relectureId && relecturesEnAttente
    ? relecturesEnAttente.find((r: RelectureEnAttente) => r.id === relectureId)
    : null;
    ...
    return (
      <div className="relecture-page">
        ...
        <div className="card empty-state">
          ...
          <h3>Aucune relecture trouvée</h3>
          <p>Cette relecture n'existe pas ou a déjà été traitée.</p>
      ...
      <section className="card">
        ...
        <div className="rules-reminder">
          ...
          <ul>
            ...
            <li>Un seul relecteur par exercice (RG4)</li>
      ...
      {relecturesEnAttente && relecturesEnAttente.length > 1 && (
        <section className="card">
          ...
          <ul className="relectures-list">
            {relecturesEnAttente
              .filter((r: RelectureEnAttente) => r.id !== relectureId)
              .map((r: RelectureEnAttente) => (
                <li key={r.id} className="relecture-item">
                  <a href={`/relecture/${r.id}`} className="relecture-link">
                    <span className="relecture-exercice">Exercice #{r.exerciceId}</span>
